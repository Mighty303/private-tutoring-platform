/** Intro to web — Beginner accordion (HTML/CSS/Tailwind), not Python exercises */
...
export function getExerciseUrl(slug) {
  if (slug.startsWith("func-discord")) return `/beginner/functions/${slug}`;
  if (slug.startsWith("func-"))        return `/beginner/functions/${slug}`;
  if (slug.startsWith("loop2-"))       return `/beginner/loops-2/${slug}`;
  if (slug.startsWith("loop-"))        return `/beginner/loops/${slug}`;
  if (slug.startsWith("nloop-"))       return `/beginner/nested-loops/${slug}`;
  if (slug.startsWith("nlist-"))       return `/beginner/nested-lists/${slug}`;
  if (slug.startsWith("web-"))         return `/beginner/${slug}`;
  if (slug.startsWith("dict-"))        return `/beginner/dictionaries/${slug}`;
  if (slug.startsWith("basics2-"))     return `/intermediate/basics-2/${slug}`;
  if (slug.startsWith("basics-"))      return `/beginner/${slug}`;
  if (slug.startsWith("heap2-"))       return `/intermediate/heaps-2/${slug}`;
  if (slug.startsWith("heap-"))        return `/intermediate/heaps/${slug}`;
  if (slug.startsWith("graph1-"))      return `/intermediate/graphs-1/${slug}`;
  if (slug.startsWith("graph2-"))      return `/intermediate/graphs-2/${slug}`;
  if (slug.startsWith("backtrack-"))   return `/intermediate/backtracking/${slug}`;
  if (slug.startsWith("tree-"))        return `/intermediate/trees/${slug}`;
  if (slug.startsWith("bsearch-"))     return `/intermediate/binary-search/${slug}`;
  if (slug.startsWith("bst-"))         return `/intermediate/bst/${slug}`;
  if (slug.startsWith("dijkstra-"))    return `/intermediate/dijkstra/${slug}`;
  return null;
}
