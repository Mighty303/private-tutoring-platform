...
import { getLessonById, basicsExercises } from "@/lib/lessons";
import MarkdownRenderer from "@/components/MarkdownRenderer";
import ExercisePlayground from "@/components/ExercisePlayground";
import { notFound } from "next/navigation";

export async function generateStaticParams() {
  const ids = getAllContentIds().filter((id) => id.startsWith("basics-"));
...
export default async function BasicsLessonPage({ params }) {
  const { id } = await params;
  const lesson = getLessonById(id);
  ...
  const currentIndex = basicsExercises.findIndex((l) => l.id === id);
  const prev = currentIndex > 0 ? basicsExercises[currentIndex - 1] : null;
  const next =
    currentIndex < basicsExercises.length - 1
      ? basicsExercises[currentIndex + 1]
      : null;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">
                {currentIndex + 1} of {basicsExercises.length}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-stretch gap-4">
            ...
            {prev ? (
              <Link
                href={`/intermediate/basics/${prev.id}`}
            ...
            {next ? (
              <Link
                href={`/intermediate/basics/${next.id}`}
