...
**Lessons system.** Lesson content is markdown under `content/` (`saturdays/` = Beginner, `sundays/` = Intermediate, `web/` = web track). Lesson routes live at `app/beginner/` and `app/intermediate/`, and the homepage/navbar link to `/beginner` and `/intermediate`. `lib/lessons.js` holds the JS catalog: lesson lists (`webLessons`, `saturdayLessons`, `IntermediateLessons`) and many per-topic exercise arrays (`loopExercises`, `functionExercises`, `graph1Exercises`, `heapExercises`, `bstExercises`, …). `lib/content.js` loads markdown by id (`getContent(id)`, `getAllContentIds()`). Rendering goes through `components/MarkdownRenderer.jsx`. Lesson pages render via dynamic routes `beginner/[id]` and `intermediate/[id]`.
