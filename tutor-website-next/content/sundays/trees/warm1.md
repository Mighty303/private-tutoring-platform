...
print(get_values(root1))

# Test 2: [7, 9]
root2 = TreeNode(7)
root2.right = TreeNode(9)
print(get_values(root2))

# Test 3: empty tree
print(get_values(None))   # []

# Test 4: single node
print(get_values(TreeNode(42)))  # [42]
```
...
Try **inorder** (left → root → right) and **postorder** (left → right → root). For a BST, what do you notice about the inorder result?

```python
def get_values_inorder(root):
    pass

def get_values_postorder(root):
    pass
```
