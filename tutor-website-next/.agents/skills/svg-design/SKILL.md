...
3. **Build an HTML preview page and auto-open it.** Follow the template in [references/editing-workflow.md](references/editing-workflow.md) for multi-size rendering, context mockups (favicon strip, nav bar), click-to-compare, and live reload. Open with `open preview.html` (macOS) or `xdg-open preview.html` (Linux).
