...
export async function generateStaticParams() {
  const ids = getAllContentIds().filter((id) => id.startsWith("dict-"));
...
export async function generateMetadata({ params }) {
  ...
  return { title: lesson ? `${lesson.title} — CS Tutor` : "Exercise — CS Tutor" };
}

export default async function DictionaryLessonPage({ params }) {
  const { id } = await params;
  const lesson = getLessonById(id);
