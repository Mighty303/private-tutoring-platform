...
import { getLessonById, loopExercises } from "@/lib/lessons";
import MarkdownRenderer from "@/components/MarkdownRenderer";
import ExercisePlayground from "@/components/ExercisePlayground";
import { notFound } from "next/navigation";

export async function generateStaticParams() {
  const ids = getAllContentIds().filter((id) => id.startsWith("loop-"));
...
export async function generateMetadata({ params }) {
  ...
  return {
    title: lesson ? `${lesson.title} — Loops` : "Loops — CS Tutor",
  };
}

export default async function LoopExercisePage({ params }) {
  const { id } = await params;
  const lesson = getLessonById(id);
  ...
  const currentIndex = loopExercises.findIndex((l) => l.id === id);
  const prev = currentIndex > 0 ? loopExercises[currentIndex - 1] : null;
  const next =
    currentIndex < loopExercises.length - 1
      ? loopExercises[currentIndex + 1]
      : null;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">
                {currentIndex + 1} of {loopExercises.length}
              </p>
            </div>
          </div>
        </div>
