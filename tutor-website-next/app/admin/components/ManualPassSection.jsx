...
export default function ManualPassSection({ students, allExerciseSlugs, grantPass, passing, passSuccess }) {
  const [showGrantPass, setShowGrantPass] = useState(false);
  ...
  return (
    <div className="mt-10">
      ...
      >
        <svg
          className={`w-4 h-4 transition-transform ${showGrantPass ? "rotate-90" : ""}`}
          fill="none" stroke="currentColor" viewBox="0 0 24 24"
        >
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        </svg>
        Manual pass (no submission)
      </button>

      {showGrantPass && (
        <div className="mt-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-5">
